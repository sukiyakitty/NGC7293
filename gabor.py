...
def save_csv(featureVectors, outfile):
    fv_mem = pd.DataFrame(featureVectors)
    fv_mem.to_csv(path_or_buf=outfile)
...
def runGabor(args):
    ...
    outfile_csv = os.path.join(temp_otfs[0], temp_otfs[1].split('.')[0] + '.csv')
...
def main():
    ...
    # Required arguments
    parser.add_argument("-infile", default=r'C:\Users\Kitty\Documents\Desktop\test\S14_Day5End_Center.png',
                        required=False)
    parser.add_argument("-outfile", default=r'C:\Users\Kitty\Documents\Desktop\test\Gabor_8_69_23.png',
                        required=False)

    parser.add_argument('-k', help='Number of clusters', type=_utils.check_positive_int, default=8, required=False)
    parser.add_argument('-gk', help='Size of the gabor kernel', type=_utils.check_positive_int, default=69,
                        required=False)
    parser.add_argument('-M', help='Size of the gaussian window', type=_utils.check_positive_int, default=23,
                        required=False)

    # Optional arguments
    parser.add_argument('-spw', help='Spatial weight of the row and columns for clustering, DEFAULT = 1', nargs='?',
                        const=1, type=_utils.check_positive_float, default=1, required=False)
    ...
    parser.add_argument('-vt', help='Variance Threshold, DEFAULT = 0.0001', nargs='?', const=0.0001, default=0.0001,
                        type=_utils.check_positive_float, required=False)
    parser.add_argument('-fi', help='Maximum number of feature images wanted, DEFAULT = 100', nargs='?', const=100,
                        default=100,
    ...
    parser.add_argument('-R', help='Energy R threshold, DEFAULT = 0.95', nargs='?', const=0.95, default=0.95,
                        type=_utils.check_positive_float, required=False)
    parser.add_argument('-siw', help='Sigma weight for gaussian smoothing, DEFAULT = 0.5', nargs='?', const=0.5,
                        default=0.5,
                        type=float, required=False)
    parser.add_argument('-c', help='Output grey? True/False, DEFAULT = False', nargs='?', const=False, default=False,
                        type=bool, required=False)
    parser.add_argument('-i', help='Print intermediate results (filtered/feature images)? True/False, DEFAULT = False',
