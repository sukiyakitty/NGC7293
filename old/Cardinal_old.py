...
from Lib_Function import get_specific_image, get_cpu_python_process, scan_dish_margin
...
def basic_task_feature_extraction(this_F_index, this_B, this_T, this_S, this_Z, this_C, this_M, write_back):
    # not using
    ...
    if this_image_path is not None and os.path.exists(this_image_path):
        ...
        cardinal_mem.loc[this_F_index, ['ana_B', 'ana_T', 'ana_S', 'ana_Z', 'ana_C', 'ana_M']] = [this_B, this_T,
                                                                                                  ...
                                                                                                  this_C, this_M]
        if write_back:
            analysis_data_mem.to_csv(path_or_buf=os.path.join(args_path, 'Analysis_Data.csv'))
            all_features.to_csv(path_or_buf=os.path.join(args_path, 'Features.csv'))
...
def finished_main_distribution_task(cardinal_mem, tiles_edge, args_B, args_T, args_S, args_Z, args_C, args_M):
    # not using
    # --- import concurrent.futures as ccf ccf.ThreadPoolExecutor---
    this_thread_pool = ccf.ThreadPoolExecutor(4)
    # finished_thread_pool = ccf.ProcessPoolExecutor(4)
    # ---begin---
    for this_F_index, this_F_row in cardinal_mem[cardinal_mem['experiment_complete'] == 1].iterrows():
                ...
                if this_M not in tiles_edge:
                    ...
                    for this_T in range(this_start_loop_T, this_end_loop_T + 1):
                        ...
                        for this_S in range(this_start_loop_S, this_end_loop_S + 1):  # note ZEN bugs
                            print('<<<--- NOW: ', this_B, this_T, this_S, 2, 1, this_M, end='')
                            # this_thread_pool.submit(fn=basic_task_feature_extraction,
                            #                         args=(this_F_index, this_B, this_T, this_S, 2, 1, this_M, True))
                            this_thread_pool.submit(
                                basic_task_feature_extraction(this_F_index, this_B, this_T, this_S, 2, 1, this_M, True))
                            # basic_task_feature_extraction(this_F_index, this_B, this_T, this_S, 2, 1, this_M, True)
                            print(' --->>>')


def outside_task_feature_extraction(path1, path2, this_B, this_T, this_S, this_Z, this_C, this_M):
    # not using
    os.system(
        "start python Task_Extract_MFeatures.py --main_path {} --this_path {} --this_B {} --this_T {} --this_S {} --this_Z {} --this_C {} --this_M {}".format(
            path1, path2, this_B, this_T, this_S, this_Z, this_C, this_M))


def finished_main_outside_task(tiles_edge, args_B, args_T, args_S, args_Z, args_C, args_M):
    # not using
    global args_path, cardinal_mem
    # --- import concurrent.futures as ccf ccf.ThreadPoolExecutor---
    # this_thread_pool = ccf.ThreadPoolExecutor(4)
    execute_number = 0
    this_process_pool = ccf.ProcessPoolExecutor(4)
    # ---begin---
    for this_F_index, this_F_row in cardinal_mem[cardinal_mem['experiment_complete'] == 1].iterrows():
        if this_F_row['ana_B'] == 0:
            this_start_loop_B = 1
                ...
                if this_M not in tiles_edge:
                    ...
                    for this_T in range(this_start_loop_T, this_end_loop_T + 1):
                        ...
                        for this_S in range(this_start_loop_S, this_end_loop_S + 1):  # note ZEN bugs
                            ...
                            this_process_pool.submit(
                                outside_task_feature_extraction(args_path, this_F_index, this_B, this_T, this_S, 2, 1,
                                                                this_M))
                            cardinal_mem.to_csv(path_or_buf=os.path.join(args_path, 'Cardinal.csv'))
                            print(' --->>>')
                            execute_number += 1
                            print('execute_number: ', execute_number)
                            temp_p_number = get_cpu_python_process()
                            print('temp_p_number: ', temp_p_number)
                            if temp_p_number > 30:
                                print('!@#$%^&*(NOW sleep!)*&^%$#@!')
                                time.sleep(2)
                                while True:
                                    if get_cpu_python_process() >= 30:
                                        time.sleep(2)
                                    else:
                                        break
                            # if execute_number % 30 == 0:  # need adjust
                            #     if get_cpu_load() >= 100:  # need adjust
                            #         print('!@#$%^&*(NOW sleep!)*&^%$#@!')
                            #         while True:
                            #             if get_cpu_load() >= 100:  # need adjust
                            #                 time.sleep(1.5)  # need adjust
                            #             else:
                            #                 break
                            # time.sleep(1.5)  # need adjust
...
def main_PCA_task():
    # not using
    print('this is PCA, and time is: ', time.time())


def main(args):
    # time.sleep(30)
    print('Cardinal Version: 0.1!')
    ...
    print('Hello Boy! Now this Program will do some Analysis of the Image that CD7 producing...')
    ...
    if args.method == 0:
        ...
        os.system(
            "start python Scheduling_2Directions.py --main_path {} --B {} --T {} --S {} --Z {} --C {} --M {} --rt_process {} --once_process {} --max_process {} --time_slice {}".format(
        ...
        os.system(
            "start python Scheduling_Column.py --main_path {} --B {} --T {} --S {} --Z {} --C {} --M {} --max_process {} --time_slice {}".format(
        ...
        os.system(
            "start python Scheduling_Sequential.py --main_path {} --B {} --T {} --S {} --Z {} --C {} --M {} --max_process {} --time_slice {} --zoom {} --overlap {} --silly {}".format(
        ...
        os.system(
            "start python Scheduling_x.py --main_path {} --B {} --T {} --S {} --Z {} --C {} --M {} --time_slice {} --zoom {} --overlap {} --analysis {}".format(
                args.path, args.B, args.T, args.S, args.Z, args.C, args.M, TIME_SLICE, zoom, args.overlap,
                args.analysis))
    else:
...
def parseArguments(argv):
    ...
    parser.add_argument('--path', type=str, nargs='?', default=r'D:\PROCESSING\CD22',
                        help='The Main Folder Path')
    parser.add_argument('--date', type=str, nargs='?', default='2019-05-19',
                        help='ZEN automatic generated date folder')
    parser.add_argument('--name', type=str, nargs='?', default='IPS-4_CD22',
    ...
    parser.add_argument('--max_process', type=int, nargs='?', default=20, help='max_process')
    ...
    parser.add_argument('--analysis', type=int, nargs='?', default=7, help='Analysis flag')
